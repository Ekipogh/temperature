    ...
    def handle(self, *args, **options):
        ...
        if options['detailed'] or not running:
            ...
            if 'consecutive_failures' in status:
                ...
                if failures > 0:
                    self.stdout.write(self.style.WARNING(f"Consecutive Failures: {failures}"))
                else:
            ...
            if 'last_successful_reading' in status and status['last_successful_reading']:
                self.stdout.write(f"Last Successful Reading: {status['last_successful_reading']}")