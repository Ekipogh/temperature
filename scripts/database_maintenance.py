...
class DatabaseMaintenance:
    ...
    def get_database_stats(self):
        ...
        if stats['total_records'] > 0:
            ...
            stats['date_range_days'] = (
                last_record.timestamp - first_record.timestamp).days

            # Records per location
            locations = Temperature.objects.values('location').distinct()
            for loc in locations:
                location_name = loc['location']
                count = Temperature.objects.filter(
                    location=location_name).count()
    ...
    def print_stats(self):
        ...
        if stats['total_records'] > 0:
            ...
            avg_bytes_per_record = stats.get(
                'file_size_bytes', 0) / stats['total_records']
            print(f"Avg Bytes/Record: {avg_bytes_per_record:.0f}")

            # Records per day
            if stats['date_range_days'] > 0:
                records_per_day = stats['total_records'] / \
                    stats['date_range_days']
    ...
    def project_future_size(self, years=50):
        ...
        for year in periods:
            ...
            future_size_mb = (
                future_records * bytes_per_record) / (1024 * 1024)
    ...
    def optimize_database(self):
        ...
        with connection.cursor() as cursor:
            ...
            try:
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_temperature_timestamp ON homepage_temperature(timestamp);")
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_temperature_location ON homepage_temperature(location);")
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_temperature_location_timestamp ON homepage_temperature(location, timestamp);")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == 'stats':
            ...
            print(
                "Usage: python database_maintenance.py [stats|project|optimize|cleanup [days]]")
    else:
