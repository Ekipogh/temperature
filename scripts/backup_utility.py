...
class DatabaseBackup:
    ...
    def __init__(self, db_path=None, backup_dir=None):
        self.db_path = Path(db_path) if db_path else Path(__file__).parent / 'db.sqlite3'
    ...
    def list_backups(self):
        ...
        backups = []
        for backup_file in sorted(self.backup_dir.glob("temperature_backup_*.sqlite3*")):
            try:
    ...
    def restore_backup(self, backup_path, confirm=True):
        ...
        if confirm:
            response = input(f"⚠️ This will overwrite the current database. Continue? (yes/no): ")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python backup_utility.py [create|list|cleanup|verify|restore] [options]")
        return
    ...
    if command == 'create':
        ...
        for backup in backups:
            print(f"  {backup['file']} - {backup['size_mb']:.1f} MB - {backup['modified']}")
